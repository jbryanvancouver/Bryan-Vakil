\message{ !name(Bryan-Vakil.tex)}\documentclass{amsart}
...
\message{ !name(Bryan-Vakil.tex) !offset(-3) }
...
Mann-Milgram \cite{Mann-Millgram-93}, and Boyer-Mann-Hurtubise-Milgram \cite{Boyer-Mann-Hurtubise-Milgram}.
...
be regarded as a non-negative integer (the degree)\footnote{For $\OGr
...
finite stabilizers, it follows that $(\alpha ,j)$ is $G(A,\phi )$
stable and thus $V^{\open}_{\mp ,n,d}\subset V^{\stable }_{\mp ,n,d}$
...
maps $BO(n)\to BO(n+1)$. Thus $BO$ is well-defined up to isomorphism
in $\HoG$.
...
\message{ !name(Bryan-Vakil.tex) !offset(-2377) }
