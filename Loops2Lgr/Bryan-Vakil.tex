...
\date{Last compiled:  \today.  Last edited: May 10th 2026}
...
Lagrangian Grassmannians and the orthogonal counterpart. We define the
...
the algebraic double loop spaces of $X_{\pm,n}$ are homotopty
...
The compliment of the open embedding
...
$\infty$-connected. {\color{red} Ravi, is this said correctly?}
...
be replaced  {\color{red} Ravi, is this said correctly?} with 
\[
...
to the natural inclusions $X_{\mp ,n}\hookrightarrow X_{+,n+1}$. In
...
\begin{lemma}\label{lem: the compliment of Vstable in V in the Sp/O
case has codim n} The compliment of the open embedding $V^{\stable}_{+
,n,d}\subset  V_{+ ,n,d}$ has codimension $n$ and the
compliment of the open embedding $V^{\stable}_{- ,n,d}\subset 
V_{- ,n,d}$ has codimension $n-1$.
Consequently, the open embeddings  $V^{\stable}_{\mp  ,n,d}\hookrightarrow
V_{\mp  ,n,d}$ are $(n-2)$-connected. 
\end{lemma}
{\color{red} Check this lemma carefully. I'd like to see the
codimension computation geometrically from the full moduli stack.}
\begin{proof}
We denote the compliment of $V^{\stable}_{\mp ,n,d}\subset V_{\mp
...
\]
Let us abbreviate $V_{\mp ,n,d}$ to $V_{\pm}$ and we 
...
Returning to $\tilde{Z}$, we see it is a subbundle of co-rank $n+d$ in
...
which proves the lemma.
\end{proof}
With the lemma proving that $\LoopTwo (X_{\pm,\infty })$ is
well-defined in $\HoG$, the isomorphisms in Proposition~\ref{prop:
Loop2(X+,inrty)=BSp(d) in HoG and same for X-} then follow from
observing that $G(A,\phi )\cong Sp(d)$ in the $X_{+}$ case and
$G(A,\phi )\cong O(d)$ in the $X_{-}$ case. 
...
connectivity. The former follows from Lemma~\ref{lem: the compliment
of Vstable in V in the Sp/O case has codim n} and the later follows
from the claim in the following:
\begin{lemma}
...
stabilizers given by the group of (blinear form preserving)
automorphisms of the orthogonal compliment of the space of $\left\{e,f
\right\}$. These stabilizers are all isomorphic to $G_{\mp}(d)$. By
the orbit-stabililer theorem we get an equivalence of stacks
\[
...
\]
which are the desired isomorphism from Proposition~\ref{prop:
