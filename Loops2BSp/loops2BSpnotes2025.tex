...
{\parskip=12pt % closing bracket is just before the bibliography 
  ...
  $\boxed{x} = x_0/x_1$.  $\boxed{p_{\infty}} = \infty = [1,0] = V(x_1)$.
...
Thus $\dim A = 2n(d-1) + 2n = 2nd$.

\point We now consider the Zariski-framed moduli space, which doesn't yet have a name. I temporarily dub it $\boxed{X(d)}$.  $\dim X(d)  = (\dim A)(\dim U)= (2dn)(2n)=4dn^2$. \label{s:dimXd}

\point The data of the Zariski-framed bundle $\cE$ is the data of $A$, plus $\al: A \rightarrow A$, and $j:A \rightarrow U$,
...
\epoint{Relationships between $(\al, j)$ and the  pairing $(\psi_\cE/\phi_\cE)$}

We unpack \eqref{eq:p20}.  We have
$$
\boxed{\psi_{A \oplus U}} : \underbrace{(A \oplus U)}_{\text{dim}=(2d+1)(2n)} \otimes (A \oplus U) \rightarrow \C[x_0, x_1]_{2d}$$
where the subscript means ``homogeneous of degree $2d$'', or equivalently,
$$
\boxed{\phi_{A \oplus U}} : A \oplus U \rightarrow (A^\vee \oplus U^\vee)[x_0, x_1]_{2d}$$
satisfying $\phi_{A \oplus U} = - \phi^\vee_{A \oplus U}$,
satisfying three conditions:
\begin{enumerate}
\item[(V)] ``vanishes on $(x-\al, -j)$ (on either factor)'' (closed condition) and
  \item[(ND)]  ``nondegenerate on the quotient for all $x \in \C$''  (open condition)
  \item[(BP)] $[x_0^{2d}] \phi_{A \oplus U} ( (a_1, u_1), (a_2, u_2)) = \psi_U(u_1, u_2)$ (base point condition, hence nondegeneracy at $x =\infty$) 
\end{enumerate}

The following is then tautological.

\tpoint{Claim} {\em This precisely parametrizes our space. Hence our space is a quasiaffine variety.}


We unpack this.  We write the components of $\phi$ out explicitly as follows.
...
$$
\phi_{A \oplus U}(  (x_0-x_1 \al) a, ja) =0 \in A^\vee \oplus U^\vee$$
for any $x \in \C$, $a \in A$.  

The $A^\vee$ component of this gives:
$$
0 = \sum x_0^{2d-1-i} x_i \left( T_{A,i} (x_0a -x_1 \al(a)) +  T_{AU,i}^\vee  (-j(a) x_1) \right)
$$
The $U^\vee$ component of this gives:
$$0 = \sum x_0^{2d-1-i} x_i \left( T_{AU,i} (x_0a -x_1 \al(a)) +  T_{U,i}^\vee  (-j(a) x_1) \right)
$$

We thus have two sequences of conditions from these. 
$$
