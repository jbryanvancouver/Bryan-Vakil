...
\]
be the Grassmanian of $n$ dimensional quotients of $U\oplus W$ with
...
$\lambda$.
\item  $\Ker (\alpha -\lambda \cdot Id_{A})\subset \im (\gamma )$ for
all $\lambda$.
\end{enumerate}
...
\end{theorem}
\begin{remark}
The Grassmanian on the left is an approximate model for $BGL(n)$ and
...
preceise in the next subsection, that there is an approximate
...
\[
\Delta =\{y_{1}x_{0}-y_{0}x_{1} \}\subset \PP^{1}\times \PP^{1}
\]
be the diagonal. Then we get an isomorphism
\[
E
q_{*}\left(p^{*}E\otimes \OO_{\Delta} \right)
\]
