...
\[
\begin{tikzcd}
0\arrow[r]& F \arrow[r]& \UL{U}\oplus \UL{W} \arrow{r}{\UL{\theta} } &E
\arrow[r]& 0
\end{tikzcd}
\]
where $E$ is a rank $n$, degree $d$ bundle on $\PP^{1}$ and
$E|_{p_{\infty}}\cong U$ via the first component of $\UL{\theta}
...
given by quotients of an algebraic group by a parabolic subgroup and
are therefore projective algebraic varieties. We will handle these two
cases simultaneously and so it will be convenient to introduce the
following notations:
\[
...
lemma is well known \cite[]{} (find a good citation in the literature)
\begin{lemma}\label{lem: lagrangian quotient condition as a ses}
The map $f:V\to L$ is a maximal isotropic or Lagrangian quotient if
...
\]
and we may re-express the condition $\UL{d}\circ J_{\pm}^{-1}\circ
\UL{f}^{\vee}=0$ as
...
algebra:\emph{ a short exact sequence with a split middle term is equivalent
...
quasi-isomorphism of the rows.}. So for example, the
